...
The country we are going to analyze is Argentina. And the critical data we focus on in this post is Confirmed Cases and Vaccinations.
...
```{r latest_arg, echo = FALSE, warning = FALSE}
...
```{r case-comp, echo = FALSE, warning = FALSE}
...
```{r stripes, echo = FALSE, warning = FALSE}
...
```{r case, echo = FALSE, warning = FALSE}
case_p1 <- covid_data %>% 
  filter(iso3c == "ARG",
         date > "2020-03-02") %>%
...
```{r vacc-start, echo = FALSE, warning = FALSE}
vacc_start <- covid_df %>% 
...
Argentina has administered over 41,305,000 COVID vaccines so far; it is the 10th country that has administered most vaccines.

```{r vacc_data, echo = FALSE, warning = FALSE}
DT::datatable(vacc_data, 
  ...
  formatCurrency('Total Vaccinations(100K dose)',
                 currency = "", 
                 interval = 3, 
                 mark = ",") %>% 
  formatCurrency('Vaccinations per 100K Population(dose)',
                 currency = "", 
                 interval = 3, 
                 mark = ",",
                 digits = 0)
...
```{r Vacc, echo = FALSE, warning = FALSE}
vac_p1 <- covid_data %>% 
  filter(iso3c == "ARG",
         date > "2020-03-02") %>%
